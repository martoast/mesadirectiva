<template>
  <div class="tier-list">
    ...
    <div v-if="tiers.length === 0" class="empty-state">
      ...
      <p class="empty-title">{{ t.noTiersYet }}</p>
      <p class="empty-subtitle">{{ t.addFirstTier }}</p>
    </div>
      ...
      <div class="tier-main">
        ...
        <div v-if="tier.sales_start || tier.sales_end" class="tier-info">
          ...
          <span v-if="tier.sales_start && !tier.sales_end">
            {{ t.salesStart }} {{ formatDate(tier.sales_start) }}
          </span>
          <span v-else-if="!tier.sales_start && tier.sales_end">
            {{ t.salesEnd }} {{ formatDate(tier.sales_end) }}
          </span>
          <span v-else>
...
const translations = {
  // Empty state
  noTiersYet: { es: 'Aún no hay niveles de boletos', en: 'No ticket tiers yet' },
  addFirstTier: { es: 'Agrega tu primer nivel para comenzar a vender boletos', en: 'Add your first tier to start selling tickets' },
  // Stats labels
  ...
  // Sales window
  salesStart: { es: 'Ventas inician', en: 'Sales start' },
